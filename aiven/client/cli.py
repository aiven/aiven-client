...
""".format(password=self.args.password))
    ...
    def project__cmks__list(self) -> None:
        ...
        if self.args.json:
            ...
            layout = ["id", "status", "provider", "resource", "default_cmk", "created_at"]
    ...
    @arg("--default-cmk", help="Mark CMK as default for all new services", action="store_true")
    @arg.json
    def project__cmks__create(self) -> None:
        ...
        response = self.client.project_cmks_create(
            ...
            resource=self.args.resource,
            default_cmk=self.args.default_cmk,
        )
        if self.args.json:
            self.print_response(response, json=self.args.json)
        else:
    ...
    def project__cmks__update(self) -> None:
        ...
        if self.args.default_cmk == "true":
            default_cmk = True
        elif self.args.default_cmk == "false":
            default_cmk = False
        response = self.client.project_cmks_update(
            project=self.get_project(),
            cmk_id=self.args.cmk_id,
            default_cmk=default_cmk,
        )
        if self.args.json:
            self.print_response(response, json=self.args.json)
        else:
